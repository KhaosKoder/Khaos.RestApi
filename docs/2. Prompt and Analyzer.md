...
Use this prompt after you run the scaffold script and have a fresh External.<ApiName> module created.

You are a senior C# engineer. Work ONLY inside the scaffolded project:

- External.<APINAME> (Refit interfaces + DTOs)
...
6) Compile mentally and SELF-CHECK against these gates:
   ...
   - Namespace starts with External.<<APINAME>>.
...
This creates:
    ...
    src/External.NedCase/ (internal Refit layer, hooks handler already wired)
    src/Domain.NedCase/ (orchestration + EF audit stubs with {{DB_SCHEMA}})
    src/Internal.NedCase/ (controller/minimal APIs)
    src/Client.NedCase/ (SDK)
    tests/Architecture.NedCase.Tests/ (layering checks)
...
Use the prompt template above with the OpenAPI source.\ Outputs must be limited to:
    ...
    External.NedCase/Models/*.cs
    External.NedCase/IMyApi.cs
...
6) Expose Internal API
    ...
    Add curated endpoints in Internal.NedCase controllers/minimal APIs.
    Return ProblemDetails with correlation id on errors.

7) Client SDK wiring

    Confirm Client.NedCase calls Internal.NedCase routes and surfaces nested error details.
...
Place them under External.<<APINAME>>/Models.

Update External.<<API_NAME>>/IMyApi.cs with 3 methods:
