...
Use this prompt after you run the scaffold script and have a fresh Khaos.RestApi.External.<ApiName> module created.

You are a senior C# engineer. Work ONLY inside the scaffolded project:

- Khaos.RestApi.External.<APINAME> (Refit interfaces + DTOs)
...
1) Generate DTO classes from the OpenAPI schemas under:
    ...
    Khaos.RestApi.External.<<APINAME>>/Models
...
2) Implement Refit interface:
    ...
    Khaos.RestApi.External.<<APINAME>>/IMyApi.cs
...
6) Compile mentally and SELF-CHECK against these gates:
   ...
   - No use of Polly or AddPolicyHandler.
    ...
    - Namespace starts with Khaos.RestApi.External.<<APINAME>>.
...
- Provide the COMPLETE updated contents of:
    ...
    - Khaos.RestApi.External.<<APINAME>>/Models/*.cs
...
This creates:
    ...
    src/Khaos.RestApi.External.NedCase/ (internal Refit layer, hooks handler already wired)
    src/Khaos.RestApi.Domain.NedCase/ (orchestration + EF audit stubs with {{DB_SCHEMA}})
    src/Khaos.RestApi.Internal.NedCase/ (controller/minimal APIs)
    src/Khaos.RestApi.Client.NedCase/ (SDK)
    tests/Khaos.RestApi.Architecture.NedCase.Tests/ (layering checks)
...
Use the prompt template above with the OpenAPI source.\ Outputs must be limited to:
    ...
    Khaos.RestApi.External.NedCase/Models/*.cs
    Khaos.RestApi.External.NedCase/IMyApi.cs
...
dotnet test tests/Khaos.RestApi.Architecture.NedCase.Tests

Fix any analyzer violations immediately (visibility, return types, no Polly).

5) Implement Domain orchestration & auditing

    Fill Khaos.RestApi.Domain.NedCase/Services/NedCaseService.cs methods.
...
6) Expose Internal API
    ...
    Add curated endpoints in Khaos.RestApi.Internal.NedCase controllers/minimal APIs.
    Return ProblemDetails with correlation id on errors.

7) Client SDK wiring

    Confirm Khaos.RestApi.Client.NedCase calls Khaos.RestApi.Internal.NedCase routes and surfaces nested error details.
...
Place them under Khaos.RestApi.External.<<APINAME>>/Models.

Update Khaos.RestApi.External.<<API_NAME>>/IMyApi.cs with 3 methods:
