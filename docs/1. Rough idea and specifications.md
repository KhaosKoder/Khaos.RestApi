...
/src
	...
	/Khaos.RestApi.Common.Abstractions     # hooks, auth handlers, options, exceptions, guards
	/Khaos.RestApi.Common.Observability    # tracing/logging abstractions
	/Khaos.RestApi.Common.Persistence      # EF Core audit persistence
	/Khaos.RestApi.Common.Analyzers        # Roslyn rules
	/Khaos.RestApi.External.<ApiX>         # internal Refit + handlers (not public)
	/Khaos.RestApi.Domain.<ApiX>           # public domain orchestration + audit
	/Khaos.RestApi.Internal.<ApiX>         # endpoints; composed in Host.Internal
	/Khaos.RestApi.Client.<ApiX>           # public SDK
	/Khaos.RestApi.Host.Internal           # web app hosting all internal endpoints

/tools

	/Khaos.RestApi.NginxConfigGen          # config→nginx.conf generator

/tests

	/Khaos.RestApi.Architecture.<ApiX>.Tests
...
    <Exec Command="dotnet run --project tools/Khaos.RestApi.NginxConfigGen --config configs/nginx.servers.json --out artifacts/nginx/$(Configuration)/nginx.conf" />
...
**Package:** Khaos.RestApi.Common.Analyzers\ **Diagnostics:**

- **CMP001** – All types in Khaos.RestApi.External.* must be **internal**.
- **CMP002** – Refit methods in Khaos.RestApi.External.* must return **Task<ApiResponse<T>>**.
- **CMP003** – **No Polly** (retry/resilience) usage in Khaos.RestApi.External.*.
- **CMP005** – Refit interfaces in Khaos.RestApi.External.* must be **internal**.
...
**Project:** /tools/Khaos.RestApi.NginxConfigGen (net8.0 console app)\ **Inputs:** configs/nginx.servers.json (algorithm, servers, passive health thresholds).\ **Output:** artifacts/nginx/<Configuration>/nginx.conf (via Directory.Build.targets).
...
- Khaos.RestApi.Client.* **must not** depend on Khaos.RestApi.External.*.
- Khaos.RestApi.Internal.* **must not** depend on Khaos.RestApi.External.*.
- Khaos.RestApi.Domain.* **must** depend on Khaos.RestApi.External.*.
...
2. **Use the LLM** to **generate DTOs + Refit methods** _inside_ Khaos.RestApi.External.<ApiName> based on OpenAPI.
