...
>
> * The `Khaos.RestApi.External.*`, `Khaos.RestApi.Domain.*`, `Khaos.RestApi.Internal.*`, and `Khaos.RestApi.Client.*` libraries
...
All names use a placeholder **`<ApiName>`** – e.g. `Khaos.RestApi.External.Foo`, `Khaos.RestApi.Domain.Foo`, etc.
...
/ApiGatewayFramework
  /src
    /Khaos.RestApi.Common.Abstractions
    /Khaos.RestApi.Common.Observability
    /Khaos.RestApi.Common.Persistence
    /Khaos.RestApi.Common.Analyzers
    /Khaos.RestApi.External.<ApiName>
    /Khaos.RestApi.Domain.<ApiName>
    /Khaos.RestApi.Internal.<ApiName>
    /Khaos.RestApi.Client.<ApiName>
    /Khaos.RestApi.Host.Internal
  /templates
    /dotnet-new-external
    /dotnet-new-domain
    /dotnet-new-internal
...
* `openapi.json` in `Khaos.RestApi.External.<ApiName>/openapi/openapi.json`, or
* `openapi.yaml` in `Khaos.RestApi.External.<ApiName>/openapi/openapi.yaml`.
...
Generated code lives under `Khaos.RestApi.External.<ApiName>`:

* All **DTOs and Refit interfaces are `internal`**.
* Namespace: `Khaos.RestApi.External.<ApiName>.Generated`.
...
`Khaos.RestApi.External.<ApiName>` exposes a single extension method:

```csharp
namespace Khaos.RestApi.External.<ApiName>;

using Khaos.RestApi.External.<ApiName>.Generated;
...
## 3. Domain Layer Contract (`Khaos.RestApi.Domain.<ApiName>`)
...
```csharp
namespace Khaos.RestApi.Domain.<ApiName>.Models;
...
```csharp
namespace Khaos.RestApi.Domain.<ApiName>.UseCases;
...
* `I<ApiName>RefitClient` (from `Khaos.RestApi.External.<ApiName>`)
* `IApiAuditRepository` (from `Khaos.RestApi.Common.Persistence`)
* Logging and tracing abstractions from `Khaos.RestApi.Common.Observability`.
...
```csharp
namespace Khaos.RestApi.Domain.<ApiName>;

using Khaos.RestApi.Domain.<ApiName>.UseCases;
using Microsoft.Extensions.DependencyInjection;
...
## 4. Internal Layer Contract (`Khaos.RestApi.Internal.<ApiName>`)
...
```csharp
namespace Khaos.RestApi.Internal.<ApiName>;
...
`Khaos.RestApi.Host.Internal` is the only **real host**:
...
## 5. Client Layer Contract (`Khaos.RestApi.Client.<ApiName>`)
...
```csharp
namespace Khaos.RestApi.Client.<ApiName>;
...
All audit persistence lives in `Khaos.RestApi.Common.Persistence`.
...
## 7. Roslyn Analyzer Suite (`Khaos.RestApi.Common.Analyzers`)
...
* **Message:** `Type '{0}' in namespace '{1}' must be declared 'internal' in Khaos.RestApi.External.* projects.`
* **Description:** Enforces that all publicly exposed types in `Khaos.RestApi.External.*` are internal to prevent leaking provider DTOs outside the boundary.
...
{
    const string source = @"
namespace Khaos.RestApi.External.Foo;
...
<Project>
  ...
  <ItemGroup>
    ...
    <PackageReference Include="Khaos.RestApi.Common.Analyzers" Version="1.0.0" PrivateAssets="all" />
  </ItemGroup>
</Project>
```
...
### 8.3 Example `.csproj` – `Khaos.RestApi.External.<ApiName>`

```xml
<Project Sdk="Microsoft.NET.Sdk">

  <PropertyGroup>
    <RootNamespace>Khaos.RestApi.External.<ApiName></RootNamespace>
...
**File:** `/tools/Khaos.RestApi.NginxConfigGen/config/apis.json`
...
**C# model in `Khaos.RestApi.NginxConfigGen`:**
...
/templates/dotnet-new-external
  ...
  /Khaos.RestApi.External.<ApiName>.csproj
...
* References to `Khaos.RestApi.Common.*` packages
...
* Steps:
  ...
  6. Generate Internal OpenAPI spec from `Khaos.RestApi.Host.Internal` (for docs).
...
jobs:
  build-test:
    ...
    steps:
      ...
      - run: dotnet run --project ./tools/Khaos.RestApi.NginxConfigGen/Khaos.RestApi.NginxConfigGen.csproj
```
...
* Packs and publishes:
  ...
  * `Khaos.RestApi.Common.Persistence`
  * `Khaos.RestApi.Common.Analyzers`
  * `Khaos.RestApi.Client.*` (if you decide they are reusable libs)
    ...
    steps:
      ...
      - uses: actions/setup-dotnet@v4
        with:
          dotnet-version: '9.0.x'
      - run: dotnet restore
      - run: dotnet pack src/Khaos.RestApi.Common.Abstractions/Khaos.RestApi.Common.Abstractions.csproj -c Release -o ./artifacts
      - run: dotnet pack src/Khaos.RestApi.Common.Analyzers/Khaos.RestApi.Common.Analyzers.csproj -c Release -o ./artifacts
      - name: Push packages
