...
>
> * The `External.*`, `Domain.*`, `Internal.*`, and `Client.*` libraries
...
All names use a placeholder **`<ApiName>`** – e.g. `External.Foo`, `Domain.Foo`, etc.
...
* `openapi.json` in `External.<ApiName>/openapi/openapi.json`, or
* `openapi.yaml` in `External.<ApiName>/openapi/openapi.yaml`.
...
Generated code lives under `External.<ApiName>`:

* All **DTOs and Refit interfaces are `internal`**.
* Namespace: `External.<ApiName>.Generated`.
...
`External.<ApiName>` exposes a single extension method:

```csharp
namespace External.<ApiName>;

using External.<ApiName>.Generated;
...
## 3. Domain Layer Contract (`Domain.<ApiName>`)
...
```csharp
namespace Domain.<ApiName>.Models;
...
```csharp
namespace Domain.<ApiName>.UseCases;
...
* `I<ApiName>RefitClient` (from `External.<ApiName>`)
* `IApiAuditRepository` (from `Common.Persistence`)
* Logging and tracing abstractions from `Common.Observability`.
...
```csharp
namespace Domain.<ApiName>;

using Domain.<ApiName>.UseCases;
using Microsoft.Extensions.DependencyInjection;
...
## 4. Internal Layer Contract (`Internal.<ApiName>`)
...
`Host.Internal` is the only **real host**:
...
## 5. Client Layer Contract (`Client.<ApiName>`)
...
All audit persistence lives in `Common.Persistence`.
...
## 7. Roslyn Analyzer Suite (`Common.Analyzers`)
...
* **Message:** `Type '{0}' in namespace '{1}' must be declared 'internal' in External.* projects.`
* **Description:** Enforces that all publicly exposed types in `External.*` are internal to prevent leaking provider DTOs outside the boundary.
...
<Project>
  ...
  <ItemGroup>
    ...
    <PackageReference Include="Common.Analyzers" Version="1.0.0" PrivateAssets="all" />
  </ItemGroup>
</Project>
```
...
### 8.3 Example `.csproj` – `External.<ApiName>`

```xml
<Project Sdk="Microsoft.NET.Sdk">

  <PropertyGroup>
    <RootNamespace>External.<ApiName></RootNamespace>
...
**File:** `/tools/NginxConfigGen/config/apis.json`
...
* Steps:
  ...
  6. Generate Internal OpenAPI spec from `Host.Internal` (for docs).
...
jobs:
  build-test:
    ...
    steps:
      ...
      - run: dotnet run --project ./tools/NginxConfigGen/NginxConfigGen.csproj
```
...
* Packs and publishes:
  ...
  * `Common.Persistence`
  * `Common.Analyzers`
  * `Client.*` (if you decide they are reusable libs)
    ...
    steps:
      ...
      - uses: actions/setup-dotnet@v4
        with:
          dotnet-version: '9.0.x'
      - run: dotnet restore
      - run: dotnet pack src/Common.Abstractions/Common.Abstractions.csproj -c Release -o ./artifacts
      - run: dotnet pack src/Common.Analyzers/Common.Analyzers.csproj -c Release -o ./artifacts
      - name: Push packages
